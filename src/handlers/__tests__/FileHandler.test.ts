...
describe('FileHandler', () => {
    ...
    describe('File Processing', () => {
            ...
            expect(mockOpenAIService.getStreamingCompletion).toHaveBeenCalledWith(
                expect.arrayContaining([
                    expect.objectContaining({
                        role: 'system',
                        content: expect.stringContaining('analyzing a document')
                    })
                ]),
                expect.any(Function),
                expect.any(Object)
        ...
        it('should handle multiple file uploads', async () => {
            ...
            expect(mockOpenAIService.getStreamingCompletion).toHaveBeenCalledTimes(2);
        });
        ...
        it('should handle different file types', async () => {
            ...
            // Verify GPT was used for all file types
            expect(mockOpenAIService.getStreamingCompletion).toHaveBeenCalledTimes(3);
            const calls = mockOpenAIService.getStreamingCompletion.mock.calls;
            expect(calls[0][0][0].content).toContain('Excel');
            expect(calls[1][0][0].content).toContain('CSV');
            expect(calls[2][0][0].content).toContain('PDF');
        });
